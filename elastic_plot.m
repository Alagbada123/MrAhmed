...
E_RCuITZ_data = load('RC_final_E_RCa_ssd141211_sensFpor.mat','RC_sens');
RC_sens_ITZ = E_RCuITZ_data.RC_sens; % Assign to a unique variable

% Load the second dataset
E_RCu_data = load('RC_final_E_RCa_ssd111111_sensFpor.mat','RC_sens');
...
exp_E_vals  = experimental_data.SSD.youngs_modulus(5, :);
exp_std_dev = experimental_data.SSD.youngs_modulus_stddev(5, :);
...
try
    strength_data_ITZ = load('RC_final_strengthRCa_ssd141211_sensFpor.mat', 'strength_values');
    strength_values_ITZ = strength_data_ITZ.strength_values;
    disp('Loaded strength data (ITZ model).');
catch ME
    warning('Could not load ITZ strength data (RC_final_strengthRCa_ssd141211_sensFpor.mat). Error: %s', ME.message);
    strength_values_ITZ = []; % Ensure variable exists but is empty
end

try
    strength_data_RCu = load('RC_final_strengthRCa_ssd111111_sensFpor.mat', 'strength_values');
    strength_values_RCu = strength_data_RCu.strength_values; % Use a different name
    disp('Loaded strength data (RCu/No ITZ model).');
catch ME
    warning('Could not load RCu/No ITZ strength data (RC_final_strengthRCa_ssd111111_sensFpor.mat). Error: %s', ME.message);
...
try
    ...
    if exist('experimental_data', 'var') && isfield(experimental_data, 'SSD') && isfield(experimental_data.SSD, 'strength_MPa') % Check for SSD CS structure
        sample_row_index = 5; % Example: Use 4th row (RC - Recycled Concrete Aggregate) - ADJUST AS NEEDED
...
if can_create_bar_charts
    ...
    bar_labels_cs_abs = {'Experimental', 'Modeled (ITZ)', 'Modeled (no ITZ)'};
