...
load('precalc_cpITZSSD3_updated.mat')
...
for wcit=1:length(req_wc)
    ...
    for xiit=1:length(req_xi)
        ...
        % DENSITIES
        rhoH2O=1000; rhohyd=2073; rhocem=3150; rho_sand=2240; rho_ragg=2242;
        sc=3.522; raggc=1.288; 
...
%% 7) OUTPUT
if choose_Eonly==1
    filename = ['RC_final_E_RCa_ssd141211_',fileoutput,'.mat'];
    save(filename,'RC_sens');
else
    filename = ['RC_final_strengthRCa_ssd111111_',fileoutput,'.mat'];
...
if choose_Eonly == 0 % Only plot strength if it was calculated
    ...
    if exist('experimental_data', 'var') && isfield(experimental_data, 'SSD') && isfield(experimental_data.SSD, 'strength_MPa')% Check for SSD CS structure
        sample_row_index = 5; % Example: Use 4th row (RC - Recycled Concrete Aggregate)
