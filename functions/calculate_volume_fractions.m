function fractions = calculate_volume_fractions(xi, materialProperties, mixDesign, Fpor, t_ITZ, Dmin, Dmax, A)
    % CALCULATE_VOLUME_FRACTIONS Calculates the volume fractions for each phase.
    %
    % Syntax:
    %   fractions = calculate_volume_fractions(xi, materialProperties, mixDesign, Fpor, t_ITZ, Dmin, Dmax, A)
    %
    % Inputs:
    ...
    % Allocate ITZ volume fraction using calculate_ITZ_volume_fraction function
    if has_coated
        Va_total = sum(f_coated); % Total volume fraction of coated aggregates
        f_coating = calculate_ITZ_volume_fraction(Va_total, Dmin, Dmax, t_ITZ, A); % ITZ volume fraction
        f_cp_remaining = f_cp - f_coating;
    else
        f_coating = 0;
        f_cp_remaining = f_cp;
    end
