%% SCRIPT TO PLOT RESULTS FROM 'precalc_extended_model_full.mat'
% This version is specifically for the dataset generated WITHOUT an Fpor loop.
clear all; clc; close all;

% --- Configuration ---
% Make sure this filename matches the one from your calculation script
filename = 'precalc_extended_model_full.mat'; 
load(filename);

% --- Define the parameters used in the calculation ---
% These MUST match the lists used to generate the .mat file
wc_list = [0.50, 0.55, 0.60];
xi_list = 0.05:0.05:1.0;
% Fpor was not a variable in this calculation, so it is implicitly 1.0
...
Fpor_plot = 1.0; % Using Fpor=1.0 as it was not a variable in the calculation
xi_plot_range = [0, xi_list];

% --- Calculate volumes for both cases using the helper function ---
...
% --- Find indices for the two cases in the output_cell ---
...
% --- Extract data from your saved 4D output_cell ---
% Note: The indexing is now output_cell(wc, xi, wa0a, alpha)
...
%% PLOT 3: INSIGHTFUL PLOT - COMPOSITION OF PORE SPACE
...
xlabel('Hydration Degree \xi [-]', 'FontSize', 12); ylabel('Porosity Volume Fraction [-]', 'FontSize', 12);
...
% Set uniform y-axis for both subplots
