...
load('precalc_cpITZOD3_updated.mat')
...
for wcit=1:length(req_wc)
    ...
    for xiit=1:length(req_xi)
        ...
        % DENSITIES
        rhoH2O=1000; rhohyd=2073; rhocem=3150; rho_sand=2240; rho_ragg=2313;
...
%% 7) OUTPUT
if choose_Eonly==1
    filename = ['RC_final_Eod111111NC_',fileoutput,'.mat'];
    save(filename,'RC_sens');
else
    filename = ['RC_final_strengthod111111NC_',fileoutput,'.mat'];
...
exp_E_vals  = experimental_data.SSD.youngs_modulus(1, :);
exp_std_dev = experimental_data.SSD.youngs_modulus_stddev(1, :);
...
if choose_Eonly == 0 % Only plot strength if it was calculated
    ...
    if exist('experimental_data', 'var') && isfield(experimental_data, 'SSD') && isfield(experimental_data.SSD, 'strength_MPa')% Check for SSD CS structure
        sample_row_index = 1; % Example: Use 4th row (RC - Recycled Concrete Aggregate)
